  ...
  <view class="v587-color-picker" @touchmove.stop.prevent>
    ...
    >
      ...
      <view
        class="sv-slider"
        :style="{
          transform: `translate(${saturationSlider - halfSilder}px, ${valueSlider - halfSilder}px)`,
        }"
      >
      </view>
    </view>
    ...
    <view class="palette">
      ...
      <view id="sliderBars" ref="sliderBars" class="slider-bars">
        ...
        >
          <view
            class="slider"
            :style="{ transform: `translate(${hueSlider - halfSilder / 2}px, -50%)` }"
        ...
        >
          ...
          <view
            class="slider"
            :style="{ transform: `translate(${alphaSlider - halfSilder / 2}px, -50%)` }"
    ...
    <view class="fav-colors">
      <view
        class="fav-colors__item"
        v-for="(value, key) in favColors"
        :key="key"
    ...
    methods: {
      ...
      moveSlide(e, component) {
        ...
        if (component === 'sv') {
          ...
          this.setSliderValue(
            'value',
            this.startTop + pageY - this.startPageY - (this.isH5 ? this.windowTop : 0)
          );
        } else {
      ...
      setSliderValue(component, value) {
        this[component + 'Slider'] = Math.min(
          Math.max(value, 0),
          ['hue', 'alpha'].indexOf(component) !== -1 ? this.sliderWidth : this.pickerWidth
        );
      },
      emitPickedColor() {
      ...
      pick(value, tag = 'HEX') {
        if (tag === 'HEX') {
          ...
          if (s.startsWith('#') && [4, 7, 9].indexOf(s.length) > -1) {
            // 满足#fff、 #ffffff、 #ffffff00
            this.keyIn = true;
          ...
          let { R: r, G: g, B: b, A: a } = this.rgbaInput;
          this.keyIn = true;
